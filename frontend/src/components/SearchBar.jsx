...
export function SearchBar({ onSearch, onInvestigate, searchResults, loading, disabled = false }) {
  ...
  const isTxid = (str) => /^[a-fA-F0-9]{64}$/.test(str)
    ...
    <div className="relative" ref={inputRef}>
      <form onSubmit={handleSubmit} className="relative w-full max-w-2xl mx-auto sm:mx-0">
        ...
        <div className={`relative transition-all duration-200 ${focused ? 'ring-2 ring-brand-500/20' : ''}`}>
          ...
          <input
            ...
            className="input pl-12 pr-12 sm:pl-14 py-3 text-base w-full"
            autoComplete="off"
          ...
          {!loading && query.length >= 2 && !hasResults && (
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2 text-warning-500">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
      ...
      {showResults && hasResults && (
        <div
          ref={resultsRef}
          id="search-results"
          className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl shadow-xl z-50 overflow-hidden max-h-96 overflow-y-auto animate-slide-down"
          role="listbox"
        >
                ...
                {searchResults.wallets.slice(0, 5).map((wallet) => (
                  <li key={wallet.value} role="option" tabIndex={0}>
                    <button
                      ...
                      className="w-full px-4 py-3 text-left hover:bg-surface-50 dark:hover:bg-surface-800/50 flex items-center gap-3 transition-colors group"
                    >
                      ...
                      <svg className="w-4 h-4 text-surface-400 dark:text-surface-500 group-hover:text-brand-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                      </svg>
                    </button>
                  </li>
                ...
                {searchResults.transactions.slice(0, 5).map((tx) => (
                  <li key={tx.value} role="option" tabIndex={0}>
                    ...
                    >
                      ...
                      <svg className="w-4 h-4 text-surface-400 dark:text-surface-500 group-hover:text-info-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                    </button>
                  </li>
      ...
      {query.length >= 2 && searchResults && searchResults.wallets?.length === 0 && searchResults.transactions?.length === 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-xl shadow-xl z-50 p-4 text-center animate-slide-down">
          <p className="text-surface-500 dark:text-surface-400 text-sm">No results found for <span className="font-mono text-surface-700 dark:text-surface-300">"{query}"</span></p>
        </div>
      )}
    </div>
  )

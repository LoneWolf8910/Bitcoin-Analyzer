...
export function RiskSection({ data }) {
  ...
  const getScoreBg = (s) => {
    if (s >= 80) return 'bg-danger-100 dark:bg-danger-900/30 border-danger-200 dark:border-danger-800'
    if (s >= 60) return 'bg-orange-100 dark:bg-orange-900/30 border-orange-200 dark:border-orange-800'
    if (s >= 30) return 'bg-warning-100 dark:bg-warning-900/30 border-warning-200 dark:border-warning-800'
    return 'bg-success-100 dark:bg-success-900/30 border-success-200 dark:border-success-800'
  }
    ...
    <div className="card p-6 animate-slide-up">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-6">
        <div className="flex items-center gap-6">
          <div className={`w-28 h-28 rounded-2xl flex items-center justify-center relative ${getScoreBg(score)}`}>
            ...
            <div className="absolute inset-0 rounded-2xl border-4 border-transparent" style={{
              borderTopColor: getScoreColor(score).replace('text-', ''),
              borderRightColor: getScoreColor(score).replace('text-', ''),
              opacity: 0.3
            }} />
          </div>
          <div>
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-surface-500 dark:text-surface-400 border-t border-surface-200 dark:border-surface-700 pt-4 lg:border-0 lg:pt-0 lg:border-l lg:pl-6">
      ...
      <div className={`rounded-xl p-5 ${getRiskLevelBg(level)} border border-surface-200 dark:border-surface-700`}>
        <h3 className={`font-semibold ${getRiskLevelColor(level)} mb-4 flex items-center gap-2`}>
        ...
        {reasons && reasons.length > 0 ? (
          <ul className="space-y-3" role="list">
            {reasons.map((reason, index) => (
              <li key={index} className="flex items-start gap-3 text-sm text-surface-700 dark:text-surface-300 group">
          ...
          <div className="flex items-center gap-3 text-surface-500 dark:text-surface-400 py-2">
            ...
            <span>No significant risk indicators detected. This wallet appears to have normal transaction patterns.</span>
          </div>
        )}
      </div>
      ...
      {factor_breakdown && factor_breakdown.length > 0 && (
        <details className="mt-6 group">
          <summary className="flex items-center justify-between cursor-pointer text-sm text-surface-500 dark:text-surface-400 hover:text-surface-700 dark:hover:text-surface-300 font-medium p-2 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors">
          ...
          <div className="mt-4 space-y-3 pt-4 border-t border-surface-200 dark:border-surface-700 animate-slide-down">
...
function FactorBadge({ label, value, color }) {
  ...
  return (
    <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border ${colorMap[color]}`}>
...
function RiskLevelBar({ score }) {
  ...
  return (
    <div className="w-40 h-2 bg-surface-200 dark:bg-surface-700 rounded-full overflow-hidden">
      <div
  ...
  return (
    <div className="bg-surface-50 dark:bg-surface-800/50 rounded-xl p-4 border border-surface-200 dark:border-surface-700">
      <div className="flex items-center justify-between mb-3">
        <h4 className="font-medium text-surface-900 dark:text-surface-100 capitalize">{formatName(name)}</h4>
      ...
      <p className="text-surface-600 dark:text-surface-400 text-sm mb-3">{explanation}</p>
      {feature_values && Object.keys(feature_values).length > 0 && (
        <details className="group">
          <summary className="text-xs text-surface-500 dark:text-surface-400 hover:text-surface-700 dark:hover:text-surface-300 cursor-pointer flex items-center gap-1.5 px-2 py-1 rounded hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors">
            ...
            <svg className="w-4 h-4 transition-transform duration-200 group-open:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
            </svg>
          </summary>
          <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
            {Object.entries(feature_values).map(([key, value]) => (
              <div key={key} className="bg-surface-100 dark:bg-surface-800 rounded-lg p-2.5 font-mono text-xs">
                <span className="text-surface-500 dark:text-surface-400 block truncate">{key}</span>
                <span className="text-surface-900 dark:text-surface-100">{typeof value === 'number' ? value.toFixed(4) : value}</span>
              </div>
            ))}
          </div>
        </details>
...
function RiskSkeleton() {
  return (
    <div className="card p-6 animate-pulse animate-shimmer">
      <div className="flex items-center gap-6 mb-6">
        <div className="w-28 h-28 rounded-2xl bg-surface-200 dark:bg-surface-700" />

...
export function RecentTransactionsTable({ transactions, loading, error }) {
  ...
  if (error) {
    return (
      <div className="card p-6 bg-danger-50 dark:bg-danger-900/20 border-danger-200 dark:border-danger-800 min-w-0">
        <div className="flex items-center gap-3 text-danger-700 dark:text-danger-300">
          ...
          <p className="break-words">Failed to load transactions: {error}</p>
        </div>
      </div>
    )
  }
  ...
  if (!transactions || transactions.length === 0) {
    return (
      <div className="card p-12 text-center min-w-0">
  ...
  return (
    <div className="card overflow-hidden min-w-0">
      <div className="p-5 border-b border-surface-200 dark:border-surface-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 min-w-0">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <span className="text-sm text-surface-500 dark:text-surface-400 font-mono px-3 py-1 bg-surface-100 dark:bg-surface-800 rounded-full flex-shrink-0">
      ...
      <div className="table-container min-w-0">
        <table className="w-full min-w-max" role="table">
          <thead>
...
function TransactionRow({ tx, walletAddress }) {
  ...
  const directionIcon = isOutgoing ? (
    ...
    <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" /></svg>
  )

  return (
    <tr className="table-row group min-w-0" tabIndex={0}>
      ...
      <td className="table-cell min-w-0">
        <span className="font-mono text-sm text-surface-700 dark:text-surface-300 truncate block max-w-[200px] group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors" title={tx.txid}>
          {truncateTxId(tx.txid)}
        </span>
      </td>
      <td className="table-cell whitespace-nowrap">
        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-full ${directionBg} flex-shrink-0`}>
          {directionIcon}
          <span>{isOutgoing ? 'OUT' : 'IN'}</span>
        </span>
      </td>
      <td className="table-cell text-right pr-4 whitespace-nowrap">
        <span className={`font-mono text-sm font-medium ${amountColor}`}>
          {isOutgoing ? '−' : '+'}{formatCompactBTC(amount)}
        </span>
      </td>
      <td className="table-cell min-w-0">
        <span className="font-mono text-sm text-surface-700 dark:text-surface-300 truncate block max-w-[180px] group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors" title={counterparty}>
          {truncateAddress(counterparty)}
        </span>
      </td>
      <td className="table-cell whitespace-nowrap">
        <span className={`inline-flex items-center px-2 py-1 text-xs rounded-full ${getWalletLabelColor(tx.wallet_label)} flex-shrink-0`}>
...
function TableSkeleton() {
  return (
    <div className="card animate-pulse animate-shimmer min-w-0" role="status" aria-label="Loading transactions">
      ...
      <div className="table-container min-w-0">
        <table className="w-full min-w-max">
          <thead>
            <tr className="table-header">
              <th className="table-header-cell" /><th className="table-header-cell" /><th className="table-header-cell" />
              <th className="table-header-cell" /><th className="table-header-cell" /><th className="table-header-cell" />
            </tr>
          </thead>
          <tbody>
            {[...Array(5)].map((_, i) => (
              <tr key={i} className="border-t border-surface-100 dark:border-surface-800">
                <td className="table-cell"><div className="h-4 bg-surface-200 dark:bg-surface-700 rounded w-28" /></td>
                <td className="table-cell"><div className="h-4 bg-surface-200 dark:bg-surface-700 rounded w-40" /></td>
                <td className="table-cell"><div className="h-4 bg-surface-200 dark:bg-surface-700 rounded w-20" /></td>
                <td className="table-cell"><div className="h-4 bg-surface-200 dark:bg-surface-700 rounded w-24" /></td>
                <td className="table-cell"><div className="h-4 bg-surface-200 dark:bg-surface-700 rounded w-32" /></td>
                <td className="table-cell"><div className="h-5 bg-surface-200 dark:bg-surface-700 rounded w-24" /></td>
              </tr>
            ))}
          </tbody>
        </table>
